...
public class startShooter extends CommandBase {
   ...
   * @param m_distance
   * The distance (in feet) away from the goal tower the robot is.
   */
  public startShooter(Shooter m_subsystem, double m_distance) {
    //Puts the parameters in the command's variables to be used around as a shortcut.
    subsystem = m_subsystem;
    

    velocity = (m_distance*Constants.SHOOTMULT)+Constants.SHOOTOFFSET;
  ...
  public void initialize() {
    ...
    //Sets the speed.
    subsystem.setShooterSpeed(Math.round(velocity));
  }
